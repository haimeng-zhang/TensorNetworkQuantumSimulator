...
function main(n::Int, hx::Number, χ::Int, χ_trunc::Int, RMPS::Int)
    ...
    #Can transfer the PEPS to GPU and do everything on GPU if you want...
    #ψ0 = CUDA.cu(ψ0)
    ...
    sz_expectations_boundarymps = nothing
    szz_expectations_boundarymps = nothing
    correlator_expectations_boundarymps = nothing
    ...
    for l = 1:nl
        ...
        if l % measure_steps == 0
            ...
            #Truncate and put into the symmetric gauge (good for bMPS stability) at the same time
            ψ = ket_network(ψψ)
            ...
            ψψ_bmps_column_aligned = TensorNetworkQuantumSimulator.BoundaryMPSCache(ψψ_trunc; message_rank = RMPS, group_sorting_function, grouping_function)
            ...
            sz_boundarymps = TensorNetworkQuantumSimulator.expect(ψψ_bmps,sz_obs)
            szzs_boundarymps = TensorNetworkQuantumSimulator.expect(ψψ_bmps,szz_obs)
            ...
            sz_expectations_boundarymps = real.(sz_boundarymps)
            szz_expectations_boundarymps = real.(szzs_boundarymps)
            #Szz on i, i is always 1
            szz_expectations_boundarymps = vcat([1.0], szz_expectations_boundarymps)
            correlator_expectations_boundarymps = [szz_expectations_boundarymps[i] - sz_expectations_boundarymps[1]*sz_expectations_boundarymps[i] for i in 1:length(szz_expectations_boundarymps)]
            push!(times, t)
            CUDA.reclaim()
            i += 1
        end
    ...
    @show correlator_expectations_boundarymps

    save_file = "/mnt/home/jtindall/ceph/Data/Pasqal/2DIsingnx$(nx)ny$(ny)Quenchh$(hx)Chi$(χ)ChiTrunc$(χ_trunc)RMPS$(RMPS)TQ1.5Expects.npz"
    npzwrite(save_file, wf_mem = wf_mem, simulation_time = simulation_time, sz_expectations_boundarymps = sz_expectations_boundarymps, szz_expectations_boundarymps = szz_expectations_boundarymps, errs = errs, times = times, correlator_expectations_boundarymps = correlator_expectations_boundarymps)
