...
using ITensorNetworks: AbstractBeliefPropagationCache, IndsNetwork, BeliefPropagationCache
using NamedGraphs
using Graphs
...
function build_vertex_unitary(s::IndsNetwork, v, ec, g, δt)
    ...
    U_actual = ITensors.exp(0.5 * δt * o, linds, prime.(linds); ishermitian = false)
    U = copy(U_actual)

    U_dict = Dictionary()
    for _v in vns
        linds = [only(s[_v]), only(s[_v]')]
        V, U = ITensors.factorize_svd(U,linds)
        set!(U_dict, _v, V)
    end
    set!(U_dict, v, U)

    U_produced = reduce(*, [U_dict[v] for v in vcat([v], vns)])
    @assert norm(U_produced - U_actual) <= 1e-14
...
function apply_vertex_unitaries(physical_unitaries, ancilla_unitaries, A_sublattice, ρ::ITensorNetwork)
    ρ = copy(ρ)
    for v in A_sublattice
        up = physical_unitaries[v]
        ua = ancilla_unitaries[v]
        for vn in vcat(neighbors(ρ ,v), [v])
            ITensorNetworks.@preserve_graph ρ[vn] = noprime(ρ[vn] * up[vn])
            ITensorNetworks.@preserve_graph ρ[vn] = noprime(ρ[vn] * ua[vn])
        end
    end

    return ITensorNetworks.combine_linkinds(ρ)
end

function apply_vertex_unitaries(unitaries, A_sublattice, ρ::ITensorNetwork)
    ρ = copy(ρ)
    for v in A_sublattice
        u = unitaries[v]
        for vn in vcat(neighbors(ρ ,v), [v])
            ITensorNetworks.@preserve_graph ρ[vn] = noprime(ρ[vn] * u[vn])
        end
    end
...
function main_heisenberg_sqrt(lattice::String,χ::Int, ny::Int, mu::Float64, δt::Float64, K::Float64, nx::Int64)
    ...
    physical_us, ancilla_us = build_vertex_unitaries(sphysical, sancilla, ec, g, δt, A_sublattice)
    ...
    for i in 1:no_trotter_steps
        ...
        ρ = apply_vertex_unitaries(physical_us, ancilla_us, A_sublattice, ρ)
        #ρ = apply_vertex_unitaries(physical_us, A_sublattice, ρ)
        #ρρ = build_bp_cache(ρ)
        #ρ = truncate(ρ; cache! = Ref(ρρ), maxdim = χ, cutoff = 1e-16)

        #ρ = apply_vertex_unitaries(ancilla_us, A_sublattice, ρ)
        ρρ = build_bp_cache(ρ)
        ρ = truncate(ρ; cache! = Ref(ρρ), maxdim = χ, cutoff = 1e-16)
...
mode, lattice, χ, ny, mu, δt, K, nx = "HeisenbergSqrt", "Hexagonal", 8, 30, 0.001, 0.0025, 1.0, 2
