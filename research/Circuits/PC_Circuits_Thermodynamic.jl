...
function _apply(gate::ITensor, ψ::ITensorNetwork, ψψ::BeliefPropagationCache, vs = ITensorNetworks.neighbor_vertices(ψ, gate); apply_kwargs, regularization = 1e-15)
    ψ = copy(ψ)
    ψψ = copy(ψψ)
    s = ITensorNetworks.siteinds(ψ)
    g, _ = induced_subgraph(ITensorNetworks.underlying_graph(ψ), vs)
...
#θ, ϕ, nlayers, maxdim = 1.1, 0.5, 10, 16
ZBPs, Fs = main_hexagonal(θ, ϕ, nlayers, maxdim)

@show ZBPs, Fs

file_name =  "/mnt/home/jtindall/ceph/Data/PCCircuits/Hexagonal/Thermodynamic/Theta"*string(θ) *"Phi"*string(ϕ) * "Nlayers" * string(nlayers) * "maxdim" * string(maxdim) * ".npz"
